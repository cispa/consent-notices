...
class Device(object):
    ...
    def __init__(self, device_serial=None, listen_port=None, is_emulator=False, output_dir=None,
    ...
    def copy_shared_prefs(self, package_name, output_path):        
        #adb -s FA6AL0308231 shell su -c cp -r /data/data/alldictdict.alldict.elru/shared_prefs /sdcard/shared_prefs
        try:            
            process = subprocess.Popen(["adb", "-s", self.serial, "shell", "su", "-c", "cp", "-r", f"/data/data/{package_name}/shared_prefs", "/sdcard/shared_prefs"], stdout=subprocess.PIPE)
            time.sleep(1)
            shell_output = self.adb.run_cmd(['pull', '/sdcard/shared_prefs', output_path])      
            process = subprocess.Popen(["adb", "-s", self.serial, "shell", "rm", "-r", "/sdcard/shared_prefs"])
            time.sleep(1)        
        except Exception as e:
            pass
