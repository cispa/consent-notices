...
class MITMPolicy(UtgBasedInputPolicy):
	"""
	DFS/BFS (according to search_method) strategy to explore UFG (new)
	"""

	def __init__(self, device, app, random_input, search_method, consent_condition, privacy_wording_file):
		super(MITMPolicy, self).__init__(device, app, random_input)
		self.logger = logging.getLogger(self.__class__.__name__)
		self.search_method = search_method

		
		self.CONSENT_NAVIGATION_DICT = {'confirm' : ['yes', 'agree', 'ok', 'continue', 'consent', 'confirm', 'understand', 'start', 'okay', 'enable', 'got it', 'go', 'join', 'next', 'understood', 'play', 'allow'], 
		...
		self.last_state = None		
		self.__first_event = True		
		self.consent_condition = consent_condition
		self.privacy_wording_file = privacy_wording_file
		

		self.found_consent_dialog = False
		self.event_manual_waiting_consent = 0
		self.number_of_scroll = 0

		self.privacy_word_set = self.get_privacy_wordings(self.privacy_wording_file)		

		shell_output = device.adb.shell(['ps', '-A'])
		if 'frida-server' not in shell_output:
			subprocess.Popen(["adb", "-s", device.serial, "shell", "su", "-c", "./data/local/tmp/frida-server&"], stdout=subprocess.PIPE)
			self.logger.info('Started frida-server!')
		else:
			self.logger.info('Frida-server has been started!')

		cmd_mitm = f'mitmdump -w {self.app.output_dir}/{self.app.package_name}.mitm --set block_global=false --set listen_port={self.device.listen_port}'
		pro_mitm = subprocess.Popen(cmd_mitm, stdout=subprocess.PIPE, shell=True, preexec_fn=os.setsid)
		
	def get_privacy_wordings(self, file_input = 'privacy_wording.json'):
		word_set = set()
	...
	def generate_event_based_on_utg(self):
		...
		if self.__first_event:
			self.__first_event = False
			self.logger.info("Trying to start the app...")
			cmd_objection = f'objection -S {self.device.serial} -g {self.app.package_name} explore --startup-command "android sslpinning disable"'
		...
		if self.event_manual_waiting_consent <= 5:
			self.event_manual_waiting_consent += 1
			return ManualEvent()
		

		self.found_consent_dialog = True
		view_to_touch = self.select_a_view(self.current_state)

		# print(self.device.get_current_activity_stack())
		# if no view can be selected, restart the app
		if view_to_touch is None:
			stop_app_intent = self.app.get_stop_intent()
			self.last_event_flag += EVENT_FLAG_STOP_APP
	...
	def is_consent_dialog(self, state):
		view_texts = []
		for view in state.views:
			if view['text'] is not None:
				view_texts.append(view['text'].lower())

		full_text = ' '.join(view_texts)				
		
		
		for privacy_word in self.privacy_word_set:		
			if privacy_word in full_text:
				return True
		return False
	...
	def select_a_view(self, state):
		...
		for view in views:					
			...
			if view_text != '':
				for nav_text in self.CONSENT_NAVIGATION_DICT[self.consent_condition]:
					if nav_text in view_text and (state.foreground_activity, view['view_str']) not in self.explored_views:									
						# print('here')
						print(nav_text)
						if not self.found_consent_dialog:
							if self.is_consent_dialog(state):								
								self.found_consent_dialog = True
								return view
						else:							
							return view
