...
DEVICE_SERIAL_PORT_DICT = {'05SAY1Y52M':1010, 'FA6AL0308231':2020, '05SAY1Y57B':3030, '017AY1S3EN':4040}
...
class DroidBot(object):
	...
	def __init__(self,
		...
		try:
			...
			self.input_manager = InputManager(
				...
				listen_port=DEVICE_SERIAL_PORT_DICT[device_serial],
				consent_group=consent_group,
				consent_navigation=consent_navigation)
		except Exception:
			import traceback
			traceback.print_exc()
			self.stop()
	...
	def is_process_running(self, process_cmd):
		for proc in psutil.process_iter():
			try:
				cmd = ' '.join(proc.cmdline())				
				if process_cmd in cmd:
					return True
			except Exception as e:          
				pass        
		return False

	def tear_down(self, listen_port):		
		mitm_cmd = f'--set block_global=false --set listen_port={listen_port}'
		while self.is_process_running(mitm_cmd):		
			for proc in psutil.process_iter():
				try:
					cmd = ' '.join(proc.cmdline())					
					if mitm_cmd in cmd:
		...
		objection_cmd = f'objection -S {self.device.serial} -g'
		while self.is_process_running(objection_cmd):		
			for proc in psutil.process_iter():
				try:
					cmd = ' '.join(proc.cmdline())					
	...
	def stop(self):
		self.enabled = False
		self.tear_down(DEVICE_SERIAL_PORT_DICT[self.device.serial])
