...
class MITMPolicy(UtgBasedInputPolicy):
	...
	def __init__(self, device, app, random_input, search_method, listen_port, consent_group, consent_navigation):
		super(MITMPolicy, self).__init__(device, app, random_input)
		self.logger = logging.getLogger(self.__class__.__name__)
		self.search_method = search_method


		self.CONSENT_GROUP_DICT = {'1-use':'we care about your privacy and data security. we keep this app free by showing ads'}
		self.CONSENT_NAVIGATION_DICT = {'1-use' : {'p1' : ['yes, continue to see relevant ads'], 'p2':['no, see ads that are less relevant', 'agree']}}

		# self.preferred_buttons = ["yes", "ok", "activate", "detail", "more", "access",
		# 						  "allow", "check", "agree", "try", "go", "next"]
		#self.preferred_buttons = ["no, see ads that are less relevant", "no see ads that are less relevant", "cancel", "disagree", "no", "no, thank you", ]
		...
		self.consent_navigation = consent_navigation
		...
		cmd_mitm = f'mitmdump -w {self.app.output_dir}/{self.app.package_name}.mitm --set block_global=false --set listen_port={listen_port}'
	...
	def generate_event_based_on_utg(self):
		...
		# select a view to click
		if not self.is_consent_dialog(self.current_state):
			return ManualEvent()
	...
	def is_consent_dialog(self, state):
		...
		return self.CONSENT_GROUP_DICT[self.consent_group] in full_text
	...
	def select_a_view(self, state):
		...
		for view in views:					
			...
			if view_text in self.CONSENT_NAVIGATION_DICT[self.consent_group][self.consent_navigation] \
				and (state.foreground_activity, view['view_str']) not in self.explored_views:
				return view
